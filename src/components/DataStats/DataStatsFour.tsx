...
interface Stats {
  image: string;
  name: string;
  symbol: string; // Add the stock symbol for each company
}
...
const DataStatsFour: React.FC = () => {
  ...
  // Function to fetch stock data for multiple symbols in a single API call
  async function fetchStockData(symbols: string[]) {
    const joinedSymbols = symbols.join(","); // Join symbols by comma for the API request
    const url = `https://api.marketstack.com/v1/eod?access_key=3495684bd2c40173fac65beaa6b98c9c&symbols=${joinedSymbols}`;
    const options = {
      method: "GET",
    };
  ...
  // Fetch stock data for all companies in a single API call
  useEffect(() => {
    const symbols = statsItems.map((item) => item.symbol); // Get all symbols
    fetchStockData(symbols).then((data) => {
      if (data) {
        setStockData(data); // Store the fetched data
      }
    });
  }, []);
            ...
            >
                ...
                <ChartEleven
                  returnRate={
                    stockData[item.symbol]
                      ? stockData[item.symbol].change_percentage
                      : 0
                  }
                />
              </div>
              <div className="mt-5.5 flex flex-col gap-1.5">
                ...
                <div className="flex items-center justify-between gap-1">
                  ...
                  <p className="font-medium text-black dark:text-white">
                    {stockData[item.symbol]
                      ? stockData[item.symbol].date.split("T")[0]
                      : "Loading..."}
                  </p>
                </div>
              </div>
